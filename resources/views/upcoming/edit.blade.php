...
@section('content')
    ...
    <form method="POST" action="{{ route('upcoming.update', ['upcoming' => $upcoming->id]) }}" class="py-4">
        @csrf
        @method('put')
        <div class="grid gap-4 mb-4 grid-cols-2">
            <div class="col-span-2">
                ...
                <input type="text" name="title" id="name"
                    ...
                    placeholder="Upcoming Event" value="{{ $upcoming->title }}">
                @error('title')
            ...
            <div class="col-span-2 ">
                ...
                <textarea id="description" rows="2"
                    ...
                    placeholder="Write event description here" name="content">{{ $upcoming->content }}</textarea>
                @error('content')
                    <x-typography.error-form>{{ $message }}</x-typography.error-form>
                @enderror
            </div>
            ...
            <div class="col-span-2">
                ...
                <textarea id="location" rows="2"
                    class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    placeholder="Write location of event here" name="location">{{ $upcoming->location }}</textarea>
                @error('location')
                    <x-typography.error-form>{{ $message }}</x-typography.error-form>
                @enderror
            </div>
